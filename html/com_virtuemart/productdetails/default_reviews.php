...
if (
    ...
    for ($num = 0; $num <= $maxrating; $num++) {
        $stars[] =
            '
				<span title="' .
            (vmText::_('COM_VIRTUEMART_RATING_TITLE') .
                $num .
                '/' .
                $maxrating) .
            '" class="vmicon ratingbox" style="display:inline-block;width:' .
            12 * $maxrating .
            'px;">
    ...
    if ($review_editable) { ?>
		...
		<form method="post"
         ...
         false
			...
			<?php
   if ($this->allowRating and $review_editable) { ?>

				<h4><?php
    echo vmText::_('COM_VIRTUEMART_WRITE_REVIEW');
    if (count($this->rating_reviews) == 0) { ?><span><?php echo vmText::_(
    'COM_VIRTUEMART_WRITE_FIRST_REVIEW'
); ?></span><?php }
    ?>
				</h4>
				<span class="step"><?php echo vmText::_(
        'COM_VIRTUEMART_RATING_FIRST_RATE'
    ); ?></span>
				<div class="rating">
...
var ausgewaehlt = false;
	...
	if (form.comment.value.length < " .
        ...
        addslashes(
            vmText::sprintf(
                'COM_VIRTUEMART_REVIEW_ERR_COMMENT2_JS',
                VmConfig::get('reviews_maximum_comment_length', 2000)
            )
        ) .
        "');
...
";
    ...
    ?>
				<span
					class="step"><?php echo vmText::sprintf(
         'COM_VIRTUEMART_REVIEW_COMMENT',
         VmConfig::get('reviews_minimum_comment_length', 100),
         VmConfig::get('reviews_maximum_comment_length', 2000)
     ); ?></span>
				<br/>
				<textarea class="virtuemart" title="<?php echo vmText::_(
        'COM_VIRTUEMART_WRITE_REVIEW'
    ); ?>"
   ...
   if ($review_editable and $this->allowReview) { ?>
					...
					<input class="highlight-button" type="submit" onclick="return( check_reviewform());"
						   name="submit_review" title="<?php echo vmText::_(
             'COM_VIRTUEMART_REVIEW_SUBMIT'
         ); ?>"
  ...
  if ($this->rating_reviews) {
    ...
    if (!empty($this->rating_reviews) && $review->published) {
        $reviews_published++; ?>
					<div class="<?php echo $color; ?>">
						<span class="date"><?php echo JHtml::date(
          $review->created_on,
          vmText::_('DATE_FORMAT_LC')
      ); ?></span>
    ...
    if ($i == $ratingsShow && !$showall) {
        ...
        if ($reviews_published >= $ratingsShow) {
            $attribute = [
                'class' => 'details',
                'title' => vmText::_('COM_VIRTUEMART_MORE_REVIEWS'),
            ];
            echo JHtml::link(
                $this->more_reviews,
                vmText::_('COM_VIRTUEMART_MORE_REVIEWS'),
                $attribute
            );
        }
        break;
